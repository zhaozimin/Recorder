...
音频只在内存中转写、转完即弃,**不产生任何 `.wav`**;文字只写入你本地的笔记文件夹。唯一的联网是首次下载模型权重(以及你主动点「检查更新」时)。

> **「夜间整理」是手动配方,不是内置功能**:笔记本就是纯 Markdown,你想用云端 Claude 把当天文字整理成日志/推文时,自己打开当天笔记、连同 [`voicelog/claude_prompt.md`](voicelog/claude_prompt.md) 贴给 Claude 即可。这一步**完全由你手动发起**,核心录音转写永远在本地、绝不自动上传。
