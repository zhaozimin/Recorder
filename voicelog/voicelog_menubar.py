...
# 内置注册朗读稿：选「人人都会读」的内容——唐诗 + 数数 + 字母 + 最简单的中英问候语。
# 声纹是 text-independent(只认音色不认内容)，舒服好读比生僻术语重要；质量驱动下念完可从头再念。
DEFAULT_ENROLL_SCRIPT = """请用平常聊天的语气，慢慢地、自然地朗读下面的内容。
