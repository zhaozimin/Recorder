...
# 内置注册朗读稿：只放「要念的句子」(引导说明在窗口标题区，不混进朗读框)。
# ECAPA 声纹 text-independent——只认音色不认内容，真正决定质量的是足够的自然语音时长(质量驱动已保证)，
# 不必刻意读数字/字母/单词。所以选最自然好读的日常口语(含一句带数字童谣，顺带带到数字但不刻意)。
DEFAULT_ENROLL_SCRIPT = """你好，我现在在录我自己的声音。
