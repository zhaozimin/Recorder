...
# 内置注册朗读稿：让用户「亲口念出这款应用怎么工作」——一举两得：
#   ① 念出来即提供声纹样本(ECAPA text-independent，内容随意，关键是足够自然语音时长，质量驱动已保证)；
#   ② 读一遍就把产品的隐私承诺(本地/不上传/即用即弃)自己说了一遍，比单纯看说明记得牢。
# 第一人称、平常口语，念着自然。可在 config 的 enroll_script 自定义。
DEFAULT_ENROLL_SCRIPT = """你好，我正在给 VoiceLog 录入我自己的声音。
这款应用全程在我这台电脑上运行，不联网、不上传，也不会读取我的任何其他数据。
它录到的声音，转成文字以后会立刻丢弃，绝不写进硬盘；记下的文字，也只留在我自己的电脑里。
记住我的音色以后，它只会把我说的话记进日志，自动忽略外放的视频、电视和旁边人的声音。
所以我可以放心地用它，记录我每天的想法和灵感。"""
...
这几句话讲的正是 VoiceLog 怎么工作、怎么保护你的隐私——
念一遍，你就同时做完了两件事：完成声纹注册，也搞懂了它怎么用。

准备好就点「开始」；采够音色会自动停，大约 20 秒。

VoiceLog · 你的声音日记，只属于你。"""
