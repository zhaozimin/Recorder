 ...
 * [OUTPUT]: 对外提供 SettingsView（标签页式设置：语言时区/保存位置/关键词/参数/配置）
 * [POS]: 设置页，1:1 复刻 desktop/src/pages/Settings.tsx（页内胶囊标签 + 内容区）
...
let PARAMS: [Param] = [
    .init(name: "切句灵敏度", min: 0.30, max: 0.90, step: 0.05, def: 0.60, isInt: false, lo: "越小越易开句（噪声易误触）", hi: "越大越严（轻声可能漏）"),
    .init(name: "最短句长 (ms)", min: 100, max: 1000, step: 50, def: 300, isInt: true, lo: "越小短噪声易进", hi: "越大短句被丢"),
    .init(name: "停顿断句 (ms)", min: 300, max: 1500, step: 50, def: 700, isInt: true, lo: "越小切得碎", hi: "越大合成长句"),
    .init(name: "近场响度门 (dBFS)", min: -60, max: -30, step: 1, def: -45, isInt: false, lo: "越小（更负）收得越远", hi: "越大只收很近"),
    .init(name: "声纹严格度", min: 0.10, max: 0.80, step: 0.01, def: 0.35, isInt: false, lo: "越小越宽松（可能混入他人）", hi: "越大越严（可能误丢你）"),
    .init(name: "短句豁免 (s)", min: 0, max: 3, step: 0.1, def: 1.2, isInt: false, lo: "越小更多短句也验", hi: "越大更多短句放行"),
    .init(name: "单句上限 (s)", min: 10, max: 60, step: 5, def: 30, isInt: true, lo: "越小切得勤", hi: "越大允许长段"),
]
...
enum SettingsTab: CaseIterable, Identifiable {
    case lang, path, keywords, params, config
    var id: Self { self }
    var title: String {
        switch self {
        case .lang: "语言和时区"; case .path: "保存位置"; case .keywords: "关键词"
        case .params: "参数设置"; case .config: "配置文件"
        }
    }
    var icon: String {
        switch self {
        case .lang: "globe"; case .path: "folder"; case .keywords: "tag"
...
struct SettingsView: View {
    @State private var tab: SettingsTab = .params
...
private struct LangTab: View {
    @State private var ui = "follow"
    @State private var primary = "follow"
    @State private var secondary = "none"
    @State private var tz = "follow"

    var body: some View {
        VStack(spacing: 0) {
            LangRow(label: "界面语言", hint: "菜单与窗口显示的语言", sel: $ui,
                    opts: [("follow", "跟随系统"), ("zh", "简体中文"), ("en", "English"), ("ja", "日本語")])
            Divider()
            LangRow(label: "主语言（转写语言）", hint: "你日常主要说的话，决定 Whisper 按什么语言识别", sel: $primary,
                    opts: [("follow", "跟随系统"), ("zh", "简体中文"), ("en", "English"), ("ja", "日本語")])
            Divider()
            LangRow(label: "辅语言（夹杂语言）", hint: "日常夹杂的外语，用来强化识别提示", sel: $secondary,
                    opts: [("none", "无"), ("zh", "简体中文"), ("en", "English"), ("ja", "日本語")])
            Divider()
            LangRow(label: "时区", hint: "时间戳与当天归属日；留空跟随系统本地", sel: $tz,
                    opts: [("follow", "跟随系统"), ("sh", "Asia/Shanghai"), ("tk", "Asia/Tokyo"), ("ny", "America/New_York")])
        }
...
private struct KeywordsTab: View {
    @State private var text = "克劳德 = Claude\nObsidian\n言壤"
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            TextEditor(text: $text)
                ...
                .frame(maxWidth: .infinity, maxHeight: .infinity)
                .background(RoundedRectangle(cornerRadius: 6).fill(Color.sunken))
            HStack { Spacer(); Button("保存关键词") {}.keyboardShortcut(.defaultAction) }
        }
        .padding(20)
        .frame(maxWidth: .infinity, maxHeight: .infinity)
        .card()
    ...
    var body: some View {
        VStack(spacing: 12) {
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        HStack {
                            HStack(spacing: 8) {
                                ...
                                Toggle("", isOn: $engine.speakerOn).toggleStyle(.switch).labelsHidden()
                                    .disabled(!engine.enrolled)
                            }
                            Spacer()
                            Button { } label: {
            ...
            HStack(spacing: 12) {
                ...
                Button { for p in PARAMS { vals[p.name] = p.def }; saved = false } label: {
                    Label("恢复默认", systemImage: "arrow.counterclockwise")
                }
                .buttonStyle(.borderless)
                Button("保存设置") { saved = true }.keyboardShortcut(.defaultAction)
            }
        }
    }
}
