...
**发布约定**：每个正式版 Release 同带双平台安装包,资产用**固定名** `VoiceLog-macOS.dmg` / `VoiceLog-Windows.exe`(不带版本号),配合 `https://github.com/zhaozimin/VoxTerra/releases/latest/download/<固定名>` 给官网写死、永远拉最新。铁律:两条链接要都生效,二者必须挂在**同一个非 prerelease 的 Latest release** 内(`latest` 只认一个非预发布 release)。Windows 版本号与 macOS 独立演进。**完整发布仪式**(版本→记账→构建→公证→发布→自检 九步)见根 `CLAUDE.md` 的 `<release>` 段。
...
- `macos/`: macOS 打包子模块(已可用)。PyInstaller → 签名 → 公证 → `.dmg`。
  ...
  - `make_icon.py`: 读 `voicelog/assets/logo_src.png`(3D 彩色母版)→ 裁满铺圆角(Apple squircle)→ 一气产 `VoiceLog_iconmaster.png` + `VoiceLog.iconset/*` + iconutil 出 `VoiceLog.icns`。管线已由「白 logo 合成深色底」反转为「成品母版裁圆角」。
...
- `windows/`: Windows 实验版(**Beta 已发布**,经 CI 构建发 Release;未签名,首启 SmartScreen 需放行)。入口 `voicelog/voicelog_win.py`。
  ...
  - `make_ico.py` / `VoiceLog.ico`: 与 make_icon.py 同源——读 3D 彩色母版裁满铺圆角,产多尺寸 .ico(exe/安装器图标)。
